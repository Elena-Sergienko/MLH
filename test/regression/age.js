...
describe('Regression. Age', function () {
    ...
    describe('Test suite for Element Age', function () {
        xit('TC-4.005 Verify that integer increases by 1 if clicking <+> on spinner button', function () {
        ...
        xit('TC-4.006 Verify that number decreases by 1 if clicking <-> on spinner button', function () {
            $(sel.inputFields.age).setValue(data.ages.n20);
        ...
        xit('TC-4.007 Verify that when input field is empty and <+> is clicked then the value is 1', function () {
            $(sel.ageClick.up).click();
            const age = $(sel.inputFields.age).getValue();
            expect(age).toEqual(exp.ages.n1);
        });

        xit('TC-4.008 Verify that input field accepts a 1-digit integer', function () {
            let age;
        ...
        xit('TC-4.008.1 Verify that input field accepts numbers', function () {
        ...
        xit('TC-4.009 Verify that input field accepts a 12-digit integer', function () {
            let result;
        ...
        xit('TC-4.010 Verify that if input value is longer than a 12-digit integer, error message appears', function () {
        ...
        xit('TC-4.010.1 Verify text alert', function () {
            $(sel.inputFields.age).setValue(data.ages.thirteen);
            $(sel.alert.age).waitForDisplayed();
        ...
        xit('TC-4.012 Verify that if input value is a negative integer, submission is not allowed', function () {
        ...
        xit('TC-4.012.1 Verify that if input value is a negative integer, error message appears', function () {
        ...
        xit('TC-4.013 Verify that if input value is a floating number, submission is not allowed ', function () {
            inputValues4(data.names.shrek, data.gender.he, data.ages.floating, data.typeOfStory.comedy);
            const submitAllowed = $(sel.createButton).isEnabled();
            expect(submitAllowed).not.toEqual(true);
        });

        xit('TC-4.013.1 Verify that if input value is a floating number, error message appears ', function () {
        ...
        xit('TC-4.014 Verify that input field doesn\'t accept a special character', function () {
